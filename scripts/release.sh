...
do_tarball() {
  ...
  src="${ROOT}/build/meson-dist/dispatch-${VERSION}.tar.xz"
  if [ -f "$src" ]; then
    cp -f "$src" "$DISTDIR/"
    echo "tarball: ${DISTDIR}/dispatch-${VERSION}.tar.xz"
  else
    echo "meson dist did not produce dispatch-${VERSION}.tar.xz" >&2
...
do_deb() {
  ...
  mkdir -p "$DISTDIR"
  for f in "${ROOT}/../dispatch_${VERSION}"-*.deb \
           "${ROOT}/../dispatch-dbgsym_${VERSION}"-*.deb; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
    echo "deb: $DISTDIR/$(basename "$f")"
  done
  for f in "${ROOT}/../dispatch_${VERSION}"-*.buildinfo \
           "${ROOT}/../dispatch_${VERSION}"-*.changes; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
  done
}
...
do_appimage() {
  ...
  linuxdeploy --appdir "$APPDIR" \
    --executable "${APPDIR}/usr/bin/dispatch" \
    --desktop-file "${APPDIR}/usr/share/applications/dispatch.desktop" \
    --icon-file "${APPDIR}/usr/share/icons/hicolor/scalable/apps/dispatch.svg" \
    $PLUGIN_ARGS \
    $EXTRA_LIB \
    --output appimage
  mkdir -p "$DISTDIR"
  for f in "${ROOT}/Dispatch-${VERSION}"-*.AppImage \
           "${ROOT}/dispatch-${VERSION}"-*.AppImage \
           "${ROOT}/build/Dispatch-${VERSION}"-*.AppImage \
           "${ROOT}"/*.AppImage; do
    [ -e "$f" ] || continue
    mv -f "$f" "$DISTDIR/"
  done
